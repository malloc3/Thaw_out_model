...
%                   smaller grid.
function [small_grid, small_RefRaster] = getmelt_specifc_region_with_file(file_path, ...
    metavariable, p1, p2, start_day, number_days, h5File)
...
%Convert intrensit coordinates to ints and flip if sizes are backwards
if intrinsic_p1_y > intrinsic_p2_y
    intrinsic_y_end = single(round(intrinsic_p1_y));
    intrinsic_y_start = single(round(intrinsic_p2_y));
else %if intrinsiz_p2_y >= intrinsiz_p1_y
    intrinsic_y_start = single(round(intrinsic_p1_y));
    intrinsic_y_end = single(round(intrinsic_p2_y));
end

if intrinsic_p1_x > intrinsic_p2_x
    intrinsic_x_end = single(round(intrinsic_p1_x));
    intrinsic_x_start = single(round(intrinsic_p2_x));
else %if intrinsiz_p2_x >= intrinsiz_p1_x
    intrinsic_x_start = single(round(intrinsic_p1_x));
    intrinsic_x_end = single(round(intrinsic_p2_x));
end

% WHEN YOU MAKE THE ARRAY FOR START AND COUNT must switch X and Y to fit
% proper ROWS and Columns.  X = Columns, Y = Rows

start_arry = [min([intrinsic_y_start, intrinsic_y_end]), min([intrinsic_x_start, intrinsic_x_end]), start_date_int];

if max(start_arry < 0)
    error('Invalid Boundaries: Start condition is outside bounds')
end

% Check here if melt values seem off.   May have accidently switched Y and
% X positions...  I think it is right though.
end_Date = start_date_int + count_date - 1; % Gotta remove one index

small_grid = h5File(intrinsic_y_start:intrinsic_y_end, intrinsic_x_start:intrinsic_x_end, start_date_int:end_Date);

%Makes new RasterReference for smaller sub grid
[row, col, ~] = size(small_grid);
